 ...
 *          false - failure
 */
bool readFileToMatrix (const std::string &filePath [[maybe_unused]], Matrix &mat [[maybe_unused]])
{
...
void mlpCli(MlpNetwork &mlp) noexcept(false) {
    ...
    // Set the default image path if no input is provided
    if (imgPath.empty()) {
        imgPath = "C:\\Users\\naama\\CLionProjects\\ex4-naamaevenoz\\plot_img"; // Change this to your default image path
    }
